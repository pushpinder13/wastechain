...
export default function IncomingWastePage() {
  ...
  const load = async () => {
    try {
      const { data } = await wasteAPI.getAll({ status: 'Delivered to Recycler', limit: 50 });
      setWaste(data.waste || data);
    } catch { toast.error('Failed to load incoming waste'); }
  ...
  return (
    <div className="flex bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="flex-1 p-8">
        <h1 className="text-3xl font-bold mb-8 text-gray-800 dark:text-white">Incoming Waste</h1>
        {loading ? (
